...
class GraphUtils(nn.Module):
    ...
    def graph_stats(self,):
        if self.key_node == None:
            ...
            g_info['Average node degree'] = f'{self.graph.num_edges/ self.graph.num_nodes:.2f}'
            ...
            for node in node_types:
                ...
                if hasattr(self.graph[node], 'train_mask'):
                    ...
                    node_data['Number of train '+ node +' nodes'] = self.graph[node].train_mask.sum().item()
                    node_data['Number of val '+ node +' nodes'] = self.graph[node].val_mask.sum().item()
                    node_data['Number of test '+ node +' nodes'] = self.graph[node].test_mask.sum().item()
                    node_data['Number of classes in '+ node +' nodes'] = self.graph[node].y.unique().size(0)
                else:
                    node_data['number of '+ node +' nodes'] = total_nodes
    ...
    def visualize_embeddings(self, graph_emb, class_mapping=None, node_type=None, emb_percent=0.1):
    ...
    def similarity_search(self, graph_emb, top_k_nbors=10, nlist=10, search_type='exact'):
        ...
        # approx search
            ...
            search_index = faiss.IndexIVFFlat(quantiser, emb_dim, nlist, faiss.METRIC_INNER_PRODUCT)
            ...
            assert search_type == 'exact' or search_type == 'approx', "pass search type either exact or approx"

        return dist, nbors

    
    # store graph embeddings inside arangodb
    def store_embeddings(self, graph_emb, collection_name=None, batch_size=100, class_mapping=None, 
        node_type=None, nearest_nbors_search=False, top_k_nbors=10, nlist=10, search_type='exact'):
